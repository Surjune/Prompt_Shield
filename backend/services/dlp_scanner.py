...
# Each entry: rule_id -> (regex_pattern, description, severity)
PATTERNS = {
    # ── Financial ─────────────────────────────────────────────────────────────
    "CREDIT_CARD": (
        r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\b",
        "Credit Card Number (Visa/MC/Amex/Discover)", "CRITICAL"
    ),
    "IBAN": (
        r"\b[A-Z]{2}[0-9]{2}[A-Z0-9]{4}[0-9]{7}(?:[A-Z0-9]{0,16})?\b",
        "International Bank Account Number (IBAN)", "CRITICAL"
    ),

    # ── Indian Government IDs ─────────────────────────────────────────────────
    "INDIA_PAN": (
        # PAN: 5 uppercase letters, 4 digits, 1 uppercase letter
        # Also catches context like "pan no. ABCDE1234F" or "pan: ABCDE1234F"
        r"\b[A-Z]{5}[0-9]{4}[A-Z]\b",
        "Indian PAN Card Number", "CRITICAL"
    ),
    "INDIA_AADHAAR": (
        # Aadhaar: 12 digits, often written with spaces as XXXX XXXX XXXX
        r"\b[2-9]{1}[0-9]{3}\s?[0-9]{4}\s?[0-9]{4}\b",
        "Indian Aadhaar Number", "CRITICAL"
    ),
    "INDIA_PASSPORT": (
        # Indian passport: 1 letter + 7 digits
        r"\b[A-PR-WY][1-9][0-9]{5}[1-9]\b",
        "Indian Passport Number", "CRITICAL"
    ),
    "INDIA_DRIVING_LICENSE": (
        # DL format: state code (2 letters) + 2 digits + year (4) + 7 digits
        r"\b[A-Z]{2}[0-9]{2}\s?(?:19|20)[0-9]{2}\s?[0-9]{7}\b",
        "Indian Driving License Number", "HIGH"
    ),
    "INDIA_VPA": (
        # UPI/VPA: user@bank handle
        r"\b[a-zA-Z0-9._-]{2,}@(?:oksbi|okicici|okhdfcbank|okaxis|ybl|ibl|axl|upi|paytm|apl|waicici|wahdfcbank)\b",
        "Indian UPI / VPA Payment Handle", "HIGH"
    ),

    # ── US Government IDs ─────────────────────────────────────────────────────
    "SSN": (
        r"\b(?!000|666|9\d{2})\d{3}[- ]?(?!00)\d{2}[- ]?(?!0000)\d{4}\b",
        "US Social Security Number (SSN)", "CRITICAL"
    ),
    ...
    "PHONE_NUMBER": (
        # Matches international (+91, +1, etc.) and Indian 10-digit mobile numbers
        r"\b(?:\+?91[\s\-]?)?[6-9][0-9]{9}\b|\b(?:\+?[1-9]{1,3}[\s\-]?)?(?:\([0-9]{1,4}\)[\s\-]?)?[0-9]{3,4}[\s\-]?[0-9]{3,4}[\s\-]?[0-9]{3,4}\b",
        "Phone / Mobile Number", "HIGH"
    ),
    "IP_ADDRESS": (
...
# Rules whose violations should be categorized as PII (not CREDENTIAL)
PII_RULE_IDS = {
    "CREDIT_CARD", "IBAN", "INDIA_PAN", "INDIA_AADHAAR", "INDIA_PASSPORT",
    "INDIA_DRIVING_LICENSE", "INDIA_VPA", "SSN", "EMAIL", "PHONE_NUMBER", "IP_ADDRESS"
}
        ...
        if matches:
            for match in matches:
                ...
                violations.append(ThreatViolation(
                    category="PII" if rule_id in PII_RULE_IDS else "CREDENTIAL",
                    rule_id=rule_id,
                    match=masked,
                    description=desc,
                    severity=severity
